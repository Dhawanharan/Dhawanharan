<div align="center">

# Dhawanharan Mahalingam

**Final Year Computer Science Student · AI & Networking Enthusiast · Sri Lanka**

<img src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&pause=1000&color=2E9EF7&center=true&vCenter=true&width=500&lines=Computer+Science+Student;Eastern+University%2C+Sri+Lanka;AI+%26+Networking+Enthusiast;Full+Stack+Developer;Problem+Solver" alt="Typing SVG" />

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/dhawanharan/)
[![Gmail](https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:dhawanmaha@gmail.com)
[![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/Dhawanharan)

</div>

---

## About Me

I'm a final-year **BSc Computer Science** student at the **Trincomalee Campus, Eastern University of Sri Lanka**, currently deepening my expertise in Artificial Intelligence, Machine Learning, and Computer Networking.

- 🏆 **Finalist** — Yarl Geek Challenge 2025 *(Yarl IT Hub)*
- 🏆 **Semi-Finalist** — SLIoT Challenge 2026 *(University of Moratuwa & SLT Mobitel)*
- 🎯 Active competitor in tech hackathons and industry challenges
- 💼 Seeking internship opportunities in software development
- 🌱 Always learning, always building

---

## Tech Stack

### Languages
<p>
  <img src="https://skillicons.dev/icons?i=python,java,cpp,javascript&theme=dark" />
</p>

### Web Development
<p>
  <img src="https://skillicons.dev/icons?i=react,html,css,mysql,mongodb&theme=dark" />
</p>

### Testing & QA
<p>
  <img src="https://skillicons.dev/icons?i=selenium&theme=dark" />
  &nbsp;<img src="https://img.shields.io/badge/TestNG-FF6C37?style=for-the-badge&logo=testng&logoColor=white" />
</p>

### Tools & Design
<p>
  <img src="https://skillicons.dev/icons?i=git,github,figma,sqlite,vscode,eclipse&theme=dark" />
  &nbsp;<img src="https://img.shields.io/badge/Cisco_Packet_Tracer-1BA0D7?style=for-the-badge&logo=cisco&logoColor=white" />
</p>

### Core Competencies
`Data Structures & Algorithms` · `Software Development` · `AI & Machine Learning` · `Computer Networking` · `Database Management`

---

## Featured Projects

### 📚 [221B Archives — Sherlock Holmes Reading Tracker](https://github.com/Dhawanharan/sherlock_holmes_website)
A premium MERN app for reading and tracking all Sherlock Holmes stories — featuring automated PDF ingestion, distraction-free reading, and real-time progress sync.

`React` `Node.js` `Express` `MongoDB`

---

### ♟️ [Chess Clock](https://github.com/Dhawanharan/countdown_clock)
A modern, touch-friendly chess clock with a settings screen for selecting time controls and a full-screen two-player timer interface.

`React Native` `TypeScript` `Node.js`

---

### 🔐 [Cryptography Utility System](https://github.com/Dhawanharan/Cryptography-Utility-System)
A comprehensive Java-based cryptography tool implementing industry-standard encryption algorithms and custom ciphers, designed for secure data operations via an intuitive CLI.

`Java` `Cryptography` `Security Algorithms` `CLI`

---

### 🎨 [Whiteboard](https://github.com/Dhawanharan/Whiteboard)
A feature-rich interactive digital whiteboard for drawing, sketching, and brainstorming — ideal for online learning and creative work.

`Python` `Tkinter`

---

### 🎓 [LMS Redesign](https://github.com/Chanuka999/LMS_redesign)
A collaborative Figma redesign of a Learning Management System interface, focusing on improved UX and modern design principles.

`Figma` `UI/UX Design`

---

### 📞 [Contact Book](https://github.com/Dhawanharan/Contact_Book)
A feature-rich contact management application with an intuitive Tkinter GUI for storing, updating, and managing contacts.

`Python` `Tkinter`

---

### 👨‍👩‍👧‍👦 [Family Members Manager](https://github.com/Dhawanharan/Family_Details)
A data management application for organizing and managing family information efficiently.

`PHP` `Database Management`

---

## GitHub Stats

<div align="center">

[![GitHub Streak](https://streak-stats.demolab.com?user=Dhawanharan&theme=radical&hide_border=true)](https://git.io/streak-stats)

</div>

<div align="center">

<img src="https://github-readme-stats.vercel.app/api/top-langs/?username=Dhawanharan&theme=radical&hide_border=true&layout=compact&cache_seconds=86400" alt="Top Languages" />

</div>

<div align="center">

[![Activity Graph](https://github-readme-activity-graph.vercel.app/graph?username=Dhawanharan&theme=react-dark&hide_border=true&area=true)](https://github.com/Dhawanharan)

</div>

---

## Certifications

**Artificial Intelligence**
- AI Fundamentals — *Cisco & IBM SkillBuild*

**Networking & IT**
- IT Essentials — *Cisco*

**Programming & Development**
- Introduction to Python — *SoloLearn*
- Java Intermediate, Introduction to Java — *SoloLearn*
- Java Basic Programs — *Great Learning*
- Introduction to C++ — *SoloLearn*
- Introduction to JavaScript — *SoloLearn*

**Web Development**
- Introduction to HTML, CSS, SQL — *SoloLearn*

**Design**
- Introduction to Figma — *SimpliLearn*

---

## Current Focus

- 🤖 Deep diving into **AI & Machine Learning**
- 🌐 Mastering **Computer Networking** concepts
- 💼 Seeking **internship opportunities** in software development
- 🤝 Open to **collaborations** on innovative projects
- 🌟 Contributing to **open source**

---

<div align="center">

**"Wear your failure as a badge of honor." — Sundar Pichai**

*Feel free to explore my repositories and star ⭐ the ones you find interesting!*

[![LinkedIn](https://img.shields.io/badge/Dhawanharan_Mahalingam-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/dhawanharan/)
[![Gmail](https://img.shields.io/badge/dhawanmaha@gmail.com-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:dhawanmaha@gmail.com)
[![GitHub](https://img.shields.io/badge/Dhawanharan-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/Dhawanharan)

</div>
